...
The cyan module was not highly correlated with any particular module. Its strongest correlation was only 0.45 with the Control treatment. Therefore, we did not plot the gene-trait significance plot. We did run the rest of the WGCNA anaysis so that we could run GO enrichment and see what gene categories were in this module.
...
Figure 2.8 Eigengene of turquoise module. Eigengenes show overall expression of the module in each sample. As expected, this module shows differing expression in samples even in the same treatment. This module is most strongly down-regulated in the heat_1.1 sample.
...
Figure 2.9 Heatmap of top 100 genes in the cyan module. Just as in the eigengene, expression patterns differ across samples even in the same treatment. Heat_1.1 is starkly different from all the others, being almost entirely down-regulated. Cool_1 sample shows most similar expression pattern to the heat samples rather than the other cool samples. 

**Lastly, we make fisher and kME files to use in GO enrichment analysis**

The output shows that our data is correct as it gives the total number of isogroups and the number of isogroups in the cool module.
```{r}
vsd <- read.csv("Bpsy_wgcna_allgenes.csv", row.names=1)
options(stringsAsFactors=FALSE)
data=t(vsd)
...
inModule[genes,1]=1
sum(inModule[,1])
write.csv(inModule,file=paste(whichModule,"cool_fisher.csv",sep=""),quote=F)
...
row.names(modkME)=row.names(allkME)
names(modkME)=modColName
write.csv(modkME,file=paste(whichModule,"cool_kME.csv",sep=""),quote=F)
...
clear differences between treatments and modules are mainly treatment based not based on cell density. mention modules correlated with average cell counts too but in same pattern as thermal stress which is likely just do to counts being correlated with treatment. Heat_1.1 is weird
